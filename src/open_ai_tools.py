...
tools = [
    ...
    },
    {
        ...
        "function": {
            "name": "make_trade",
            "description": "Make a trade for the given coin.",
            "parameters": {
                "type": "object",
                "properties": {
                    "coin": {"type": "string"}
    ...
    # New tool for retrieving crypto news context.
    {
        "type": "function",
        "function": {
            "name": "get_context",
            "description": "Retrieve the latest crypto news context (top articles) for Bitcoin, Ethereum, and Ripple.",
            "parameters": {
                "type": "object",
                "properties": {},
                "additionalProperties": False
...
# Process each tool call and handle them.
for tool_call in tool_calls:
    if tool_call.function.name == "compare_trends":
        ...
        result = make_trade(coin_name)
    elif tool_call.function.name == "get_context":
        # Unpack the tuple from get_crypto() and join the output string array into a single string.
        _, result_list = crypto_news.get_crypto()
        result = "\n".join(result_list)
    else:
